...
async function canAccessLocalOnlyRoute(request) {
  ...
  // ModelScope compat: allow authenticated dashboard users (JWT cookie) to manage
  // Cloudflare quick-tunnel from the Studio browser (no loopback/CLI token there).
  // Heavy/install paths stay local-only (checked below via pathname).
  try {
    const path = request.nextUrl?.pathname || "";
    const SAFE_REMOTE_TUNNEL_PATHS = ["/api/tunnel/enable", "/api/tunnel/disable", "/api/tunnel/status"];
    if (SAFE_REMOTE_TUNNEL_PATHS.some((s) => path === s || path.startsWith(s + "/"))) {
