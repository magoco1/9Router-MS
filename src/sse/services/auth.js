...
export function extractApiKey(request) {
  // Check Authorization header first (may be stripped by ModelScope gateway)
  const authHeader = request.headers.get("Authorization");
  if (authHeader?.startsWith("Bearer ")) {
    return authHeader.slice(7);
  }

  // ModelScope-safe alternatives (gateway does not strip these)
  const candidates = [
    request.headers.get("x-api-key"),
    request.headers.get("x-goog-api-key"),
    request.headers.get("x-9router-key"),
  ];
  ...
  // /v1/models?api_key=KEY or ?key=KEY
  try {
